...
5. 调用`complete_agent_run`结束。

推荐链路：

`start_agent_run → diagnose_priority_orders → create_task_draft → create_approval_request → complete_agent_run`

只有在调试、解释单笔证据或组合工具失败时，才使用原子工具。
