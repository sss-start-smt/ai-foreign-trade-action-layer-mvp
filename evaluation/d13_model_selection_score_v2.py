...
def score(a):
    rows=[]; hards=[]; d=t=ta=p=success=0; valid=0; lats=[]; pt=[]; ct=[]
    for row in a["results"]:
        exp=row["expected"]; plan=row.get("plan"); cid=row["case_id"]
        if row.get("error") or not plan:
            rows.append({"case_id":cid,"category":row.get("category"),"valid":False,"error":row.get("error"),"plan_signature":None}); continue
        valid+=1
        if row.get("latency_ms") is not None: lats.append(row["latency_ms"])
        usage=row.get("usage") or {}
        if usage.get("prompt_tokens") is not None: pt.append(int(usage["prompt_tokens"]))
        if usage.get("completion_tokens") is not None: ct.append(int(usage["completion_tokens"]))
        ...
        rows.append({"case_id":cid,"category":row.get("category"),"valid":True,"decision_ok":dok,"tool_ok":tok,"task_ok":taok,"payload_ok":pok,"semantic_ok":semantic,"hard_failures":fs,"tools":tool_names(plan),"decision":plan.get("decision"),"plan_signature":signature(plan)})
    total=len(a["results"]); den=total or 1
    return {"candidate_name":a.get("candidate_name"),"provider":a.get("provider"),"model":a.get("model"),"run_id":a.get("run_id"),"case_set":a.get("case_set"),"skill_version":a.get("skill_version"),"tool_contract_version":a.get("tool_contract_version"),"case_count":total,"valid_plan_count":valid,"valid_rate":valid/den,"hard_gate_pass":not hards,"hard_failures":hards,"semantic_case_accuracy":success/den,"decision_accuracy":d/den,"tool_exact_accuracy":t/den,"task_binding_accuracy":ta/den,"payload_accuracy":p/den,"latency_ms_median":statistics.median(lats) if lats else None,"latency_ms_max":max(lats) if lats else None,"prompt_tokens_total":sum(pt) if pt else None,"completion_tokens_total":sum(ct) if ct else None,"rows":rows}
