...
IMPORT_CSS = r''':root{font-family:Inter,"PingFang SC","Microsoft YaHei",sans-serif;color:#172033;background:#f4f7fb}*{box-sizing:border-box}body{margin:0}header{height:88px;background:#fff;border-bottom:1px solid #e5eaf2;display:flex;align-items:center;gap:22px;padding:0 5vw;position:sticky;top:0;z-index:5}header h1{font-size:22px;margin:0 0 4px}header p{margin:0;color:#6c7690;font-size:13px}.back{color:#3669e8;text-decoration:none;font-weight:600}.version{margin-left:auto;background:#eef3ff;color:#3669e8;padding:6px 10px;border-radius:999px;font-size:12px}main{max-width:1180px;margin:28px auto;padding:0 20px 80px}.panel{background:#fff;border:1px solid #e5eaf2;border-radius:16px;padding:24px;margin-bottom:18px;box-shadow:0 8px 24px rgba(29,55,105,.05)}.hidden{display:none!important}.step-title{display:flex;gap:14px;align-items:flex-start}.step-title>b{width:32px;height:32px;display:grid;place-items:center;border-radius:10px;background:#3669e8;color:#fff}.step-title h2{margin:1px 0 5px;font-size:18px}.step-title p{margin:0;color:#768099;font-size:13px}.upload-row{display:flex;gap:12px;margin-top:20px}.file-box{flex:1;border:1px dashed #a9b7d0;background:#f8faff;border-radius:12px;height:54px;display:flex;align-items:center;padding:0 16px;color:#536079;cursor:pointer}.file-box input{display:none}button{border:0;border-radius:10px;padding:12px 18px;font-weight:700;cursor:pointer}.primary{background:#3669e8;color:#fff}.primary:disabled{opacity:.5;cursor:not-allowed}.secondary{background:#eef3ff;color:#315cc4}.template-links{display:flex;gap:16px;margin-top:12px}.template-links a{font-size:13px;color:#3669e8}.mapping-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:20px 0}.mapping-item{border:1px solid #e5eaf2;border-radius:10px;padding:11px}.mapping-item label{font-size:12px;color:#6c7690;display:block;margin-bottom:6px}.mapping-item select{width:100%;border:1px solid #d7deeb;border-radius:8px;padding:9px;background:#fff}.summary{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin:20px 0}.summary-card{border:1px solid #e5eaf2;border-radius:12px;padding:14px}.summary-card span{display:block;color:#6c7690;font-size:12px}.summary-card strong{display:block;font-size:24px;margin-top:5px}.toolbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;color:#6c7690;font-size:13px}.toolbar select{padding:7px 10px;border:1px solid #d7deeb;border-radius:8px}.table-wrap{overflow:auto;border:1px solid #e5eaf2;border-radius:12px}table{width:100%;border-collapse:collapse;min-width:980px}th,td{text-align:left;padding:12px;border-bottom:1px solid #edf0f5;font-size:13px;vertical-align:top}th{background:#f8faff;color:#5d6881}.badge{display:inline-flex;padding:4px 8px;border-radius:999px;font-size:11px;font-weight:700}.NEW{background:#e8f7ef;color:#13834d}.UPDATE{background:#fff4df;color:#a96500}.DUPLICATE{background:#eef1f6;color:#68738a}.ERROR{background:#ffe9e9;color:#c43333}.detail{max-width:340px;color:#68738a}.detail div{margin-bottom:4px}.commit-box{display:flex;align-items:end;gap:14px;margin-top:18px;flex-wrap:wrap}.commit-box label{display:flex;flex-direction:column;gap:6px;font-size:12px;color:#6c7690}.commit-box input{width:280px;padding:10px;border:1px solid #d7deeb;border-radius:8px}.commit-box p{width:100%;margin:0;color:#7a849a;font-size:12px}#commit-result h2{margin-top:0}.result-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.result-grid div{background:#f8faff;border-radius:12px;padding:16px}.result-grid strong{display:block;font-size:24px}#toast{position:fixed;right:24px;bottom:24px;background:#172033;color:#fff;padding:12px 16px;border-radius:10px;opacity:0;transform:translateY(10px);transition:.2s;pointer-events:none}#toast.show{opacity:1;transform:none}@media(max-width:800px){header{height:auto;padding:16px 18px;flex-wrap:wrap}.version{margin-left:0}.mapping-grid{grid-template-columns:1fr}.summary{grid-template-columns:repeat(2,1fr)}.upload-row{flex-direction:column}.result-grid{grid-template-columns:repeat(2,1fr)}}'''
...
ENTRY_JS = r'''(()=>{const add=()=>{if(document.querySelector('[data-excel-import-entry]'))return;const a=document.createElement('a');a.href='/import-orders';a.dataset.excelImportEntry='1';a.textContent='批量导入';a.title='Excel/CSV批量导入订单';a.style.cssText='display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:10px;text-decoration:none;color:inherit;font-weight:600;';const icon=document.createElement('span');icon.textContent='⇧';icon.style.cssText='width:24px;height:24px;border-radius:7px;background:#eef3ff;color:#3669e8;display:grid;place-items:center;font-weight:800';a.prepend(icon);const targets=['aside nav','.sidebar nav','.sidebar-menu','.nav-list','[data-nav]'];let target=null;for(const s of targets){target=document.querySelector(s);if(target)break}if(target){target.appendChild(a)}else{a.style.cssText+='position:fixed;right:18px;bottom:18px;background:#fff;border:1px solid #dfe6f2;box-shadow:0 8px 24px rgba(0,0,0,.12);z-index:9999;color:#26334d';document.body.appendChild(a)}};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',add):add();setTimeout(add,1200)})();'''
...
def register_excel_import_patch(app: FastAPI) -> None:
    ...
    app.add_middleware(ImportEntryMiddleware)
